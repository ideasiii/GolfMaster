...
public class YardageBook {
	...
	private List<YardageData> queryData(String sql) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				yardageData.setCount(rs.getInt("count"));
	...
	private String getQuerySql(ParamData paramData) throws Exception {
		try {
			...
			sql.append(
					"select ClubType,count(*) as 'count', min(CarryDistFt) as 'mix', round(avg(CarryDistFt),4) as 'avg', max(CarryDistFt) as 'max' from golf_master.shot_data");
			sql.append(" where player = '" + paramData.getPlayer() + "' ");

			/*
			 * if(paramData.getdMin() > 0 && paramData.getdMax() > 0) {
			 * sql.append(" and (CarryDistFt > " + (paramData.getdMin() * 3)
			 * +" and CarryDistFt < " + (paramData.getdMax() * 3) + ") "); }
			 */

			if (paramData.getdMin() > 0) {
				sql.append(" and (CarryDistFt >= " + paramData.getdMin() + ") ");
			}
			if ((paramData.getStartDate() != null && !paramData.getStartDate().isEmpty())
					&& (paramData.getEndDate() != null && !paramData.getEndDate().isEmpty())) {
				sql.append(" and (Date >= '" + paramData.getStartDate() + "' and Date <= '" + paramData.getEndDate()
						+ "') ");
			}

			sql.append(" group by ClubType order by ClubType ");
